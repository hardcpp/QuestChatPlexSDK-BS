...
static std::string operator+(std::string_view p_Left, std::u16string_view p_Right)
{
    ...
    auto l_Result = std::string(p_Left) + l_Converted.to_bytes(p_Right.begin(), p_Right.end());
...
{
    std::wstring_convert<std::codecvt_utf8_utf16<char16_t>, char16_t> l_Converted;
    auto l_Result = std::u16string(p_Left) + l_Converted.from_bytes(p_Right.begin(), p_Right.end());
