...
namespace CP_SDK::Unity {
    ...
    bool TextureRaw::Load(::Array<uint8_t>* p_Bytes, int& p_Width, int& p_Height, std::vector<Color>& p_Pixels)
    {
        p_Width     = 0;
        p_Height    = 0;
        ...
        p_Pixels.resize(p_Width * p_Height);

        for (auto l_I = 0; l_I < (p_Width * p_Height); ++l_I)
        {
            auto l_SrcPixel = &l_STBIBuffer[l_I * 4];
            p_Pixels[l_I] = Extensions::ColorU::Convert(Color32(l_SrcPixel[0], l_SrcPixel[1], l_SrcPixel[2], l_SrcPixel[3]));
        }
    ...
    void TextureRaw::FastGaussianBlur(int p_InWidth, int p_InHeight, std::vector<Color>& p_InPixels, int p_Radius)
    {
    ...
    void TextureRaw::Multiply(std::vector<Color>& p_ImageA, std::vector<Color>& p_ImageB)
    {
        ...
        for (auto l_I = 0; l_I < p_ImageA.size(); ++l_I)
            p_ImageA[l_I] = p_ImageA[l_I] * p_ImageB[l_I];
    }
    ...
    std::vector<Color> TextureRaw::ResampleAndCrop(int p_InWidth, int p_InHeight, std::vector<Color>& p_InPixels, int p_TargetWidth, int p_TargetHeight, float p_YOffsetRel)
    {
        ...
        for (int l_Y = 0; l_Y < p_TargetHeight; ++l_Y)
        {
            for (int l_X = 0; l_X < p_TargetWidth; ++l_X)
            {
                ...
                /// Bilinear filtering
                auto l_C11 = p_InPixels[l_FX + p_InWidth * l_FY];
                auto l_C12 = p_InPixels[l_FX + p_InWidth * l_CY];
                auto l_C21 = p_InPixels[l_CX + p_InWidth * l_FY];
                auto l_C22 = p_InPixels[l_CX + p_InWidth * l_CY];
    ...
    void TextureRaw::GaussianBlur4(int p_Width, int p_Height, std::vector<Color>& p_InPixels, int p_Radius)
    {
    ...
    void TextureRaw::BoxBlur4(int p_Width, int p_Height, std::vector<Color>& p_InPixels, int p_Radius)
    {
        BoxBlurH4(p_Width, p_Height, p_InPixels, p_Radius);
        BoxBlurT4(p_Width, p_Height, p_InPixels, p_Radius);
    }
    void TextureRaw::BoxBlurH4(int p_Width, int p_Height, std::vector<Color>& p_InPixels, int p_Radius)
    {
        ...
        for (auto l_I = 0; l_I < p_Height; ++l_I)
        {
            ...
            auto l_FV  = p_InPixels[l_TI];
            auto l_LV  = p_InPixels[l_TI + p_Width - 1];
            auto l_Val = (p_Radius + 1) * l_FV;

            for (auto l_J = 0; l_J < p_Radius; ++l_J)
                l_Val = l_Val + (p_InPixels[l_TI + l_J]);

            for (auto l_J = 0; l_J <= p_Radius; ++l_J)
            {
                l_Val = l_Val + (p_InPixels[l_RI++] - l_FV);
                p_InPixels[l_TI++] = l_Val * l_Mult;
            }

            for (auto l_J = p_Radius + 1; l_J < p_Width - p_Radius; ++l_J)
            {
                l_Val = l_Val + (p_InPixels[l_RI++] - p_InPixels[l_LI++]);
                p_InPixels[l_TI++] = l_Val * l_Mult;
            }

            for (auto l_J = p_Width - p_Radius; l_J < p_Width; ++l_J)
            {
                l_Val = l_Val + (l_LV - p_InPixels[l_LI++]);
                p_InPixels[l_TI++] = l_Val * l_Mult;
            }
        };
    }
    void TextureRaw::BoxBlurT4(int p_Width, int p_Height, std::vector<Color>& p_InPixels, int p_Radius)
    {
        auto l_Mult = (float)1 / (p_Radius + p_Radius + 1);
        for (auto l_I = 0; l_I < p_Width; l_I++)
        {
            ...
            auto l_FV  = p_InPixels[l_TI];
            auto l_LV  = p_InPixels[l_TI + p_Width * (p_Height - 1)];
            auto l_Val = (p_Radius + 1) * l_FV;

            for (auto l_J = 0; l_J < p_Radius; ++l_J)
                l_Val = l_Val + (p_InPixels[l_TI + l_J * p_Width]);
