        ...
        )
    {
        ...
        try
        {
            ...
            for (int l_RetryI = 1; l_RetryI <= self->MaxRetry; l_RetryI++)
            {
                ...
                TransferContext l_TransferContext { &progress, &token };
                ...
                curl_easy_setopt(l_ScopedCURL.Instance, CURLOPT_XFERINFOFUNCTION,
                    +[] (void* p_Client, curl_off_t p_DownloadTotal, curl_off_t p_DownloadNow, curl_off_t, curl_off_t) -> int
                    {
                        ...
                        try
                        {
                            if (l_Context->Token->get_IsCancellationRequested())
                                return 1;

                            if (l_Context->Progress->IsValid())
                            {
                                auto l_Percentage = p_DownloadTotal > 0
                                    ? static_cast<float>(p_DownloadNow) / static_cast<float>(p_DownloadTotal)
                                    : 0.0f;

                                if (isnan(l_Percentage))
                                    l_Percentage = 0.0f;

                                try { l_Context->Progress->Invoke(l_Percentage); } catch (const std::exception&) { }
                            }
                        }
                        catch (...)
                        {
                ...
                if (!l_Reply->IsSuccessStatusCode())
                {
                    ...
                    while (!token.get_IsCancellationRequested() && std::chrono::steady_clock::now() < l_RetryUntil)
